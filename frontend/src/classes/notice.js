  ...
  clickHandler(e) {
    ...
    if (action) {
      ...
      switch (action) {
        ...
        case "apply": {
					apply('Введите номер задания', input())
						.then(data => {
							if (data) {
        ...
        case "delete": {
          apply('Вы точно хотите удалить это уведомление?')
						.then(data => {
							if (data) {
								this.noticeList = this.noticeList.filter(
									(item) => item.id !== +content
                ...
                item.id
                      ...
                      item.sender.position > item.recipient.position
                        ...
                        : "red"
								...
								<div class="btn-container flex-row">
										<div class="btn apply" data-action="swap" data-content=${item.id} data-src="${item.sender.id}|${
                  item.recipient.id
                }|${item.Q.hash}">Принять</div>
	...
	apply('Введите номер задания', input())
		.then(data => {
			if (data) {
				...
				console.log(`student ${id} with app ${data} added to Q hash:${hash}`);
			}
		})
}
	...
	return new Promise((resolve, reject) => {
		...
		$apply.addEventListener('click', e => {
			...
			if (action) {
				...
				if (action === 'ok') {
					if (content) {
						const value = document.getElementById('appNumber').value
						if (value) {
							resolve(value)
						}
					}
